...
function register() {
  const res = http.post(`${BASE}/api/users/register`, JSON.stringify({
...
function login() {
  ...
  const res = http.post(`${BASE}/api/auth/login`, JSON.stringify(TEST_USER), {
    headers: { 'Content-Type': 'application/json' },
  });
...
export default function () {
  ...
  // 3. my tickets
  const tickets = http.get(`${BASE}/api/tickets/my`, headers);
